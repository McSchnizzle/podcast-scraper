...
def prep_databases(youtube_db, rss_db, transcripts_dir, mode, make_dummy, no_backup):
    ...
    # Prune YT
    if mode == "archive":
        exec_sql(youtube_db, f"UPDATE episodes SET status='archived' WHERE id NOT IN ('{y1}','{y2}');")
    ...
    for yid in (y1, y2):
        ...
        exec_sql(youtube_db, "UPDATE episodes SET status='transcribed', transcript_path=? WHERE id=?;", (tpath, yid))
        print(f"[state] YouTube {yid} -> transcribed, transcript_path={tpath}")
        if make_dummy:
            write_dummy(tpath, content=f"Dummy transcript for {yid}. Short snippet for testing the digest pipeline.")
    ...
    exec_sql(rss_db, "UPDATE episodes SET status='downloaded', transcript_path=NULL WHERE id=?;", (rid,))
    print(f"[state] RSS {rid} -> downloaded (will be transcribed during run)")

    # Summaries
    y_rows = query(youtube_db, "SELECT id, status, transcript_path, digest_topic, digest_date FROM episodes ORDER BY id;")
    r_rows = query(rss_db, "SELECT id, status, transcript_path, digest_topic, digest_date FROM episodes ORDER BY id;")
...
def main():
    ap = argparse.ArgumentParser(description="Auto-prep DBs for testing: newest 2 YouTube + newest 1 RSS")
    ap.add_argument("--youtube-db", default="youtube_transcripts.db", help="Path to youtube_transcripts.db")
    ap.add_argument("--rss-db", default="podcast_monitor.db", help="Path to podcast_monitor.db")
    ap.add_argument("--transcripts-dir", default="transcripts", help="Where to put dummy transcripts")
    ap.add_argument("--mode", choices=["archive", "delete"], default="archive", help="Archive or delete others (default: archive)")
    ap.add_argument("--no-backup", action="store_true", help="Skip backups")
    ap.add_argument("--no-dummy", action="store_true", help="Do not create dummy transcript files")
    args = ap.parse_args()
