    ...
    try:
        ...
        try:
            ...
            if "already exists" in e.stderr:
                print(
                    f"📦 Release {release_tag} exists, uploading to existing release..."
                )
                cmd = ["gh", "release", "upload", release_tag, mp3_file, "--clobber"]
                result = subprocess.run(cmd, capture_output=True, text=True, check=True)
            else:
                raise
        ...
        print(
            f"🔗 Release URL: https://github.com/McSchnizzle/podcast-scraper/releases/tag/{release_tag}"
        )
        return True
