...
class ProductionRSSGenerator:
    ...
    def generate_rss(self, output_file: str = "daily-digest.xml", max_items: int = 100) -> bool:
        ...
        try:
            ...
            for episode_info in digest_episodes:
                try:
                    ...
                    logger.error(f"❌ Failed to add episode {episode_info.get('topic', 'unknown')}: {e}")
                    continue
    ...
    def _get_digest_episodes(self, max_items: int) -> List[Dict[str, Any]]:
        ...
        for digest_key, digest_data in digest_files.items():
            try:
                ...
                episode_ids = self._get_episode_ids_for_digest(digest_data['topic'], file_date)
                
                # Generate stable GUID based on content, not runtime
                stable_guid = get_stable_guid(digest_data['topic'], episode_ids, file_date)
                ...
                episode_info = {
                    ...
                    'file_size': file_size,
                    'guid': stable_guid,
                    'title': self._generate_episode_title(digest_data['topic'], file_date),
    ...
    def _get_episode_ids_for_digest(self, topic: str, date: datetime) -> List[str]:
        ...
        try:
            ...
            return sorted(list(set(episode_ids)))  # Remove duplicates and sort for consistency
    ...
    def _add_channel_metadata(self, channel: ET.Element):
        ...
        ET.SubElement(channel, 'title').text = sanitize_xml_content(channel_info['title'])
        ET.SubElement(channel, 'description').text = sanitize_xml_content(channel_info['description'])
        ET.SubElement(channel, 'link').text = channel_info['link']
        ET.SubElement(channel, 'language').text = channel_info['language']
        ET.SubElement(channel, 'copyright').text = sanitize_xml_content(channel_info['copyright'])
        ...
        ET.SubElement(channel, '{http://www.itunes.com/dtds/podcast-1.0.dtd}author').text = channel_info['author']
        ET.SubElement(channel, '{http://www.itunes.com/dtds/podcast-1.0.dtd}summary').text = sanitize_xml_content(channel_info['summary'])
        ET.SubElement(channel, '{http://www.itunes.com/dtds/podcast-1.0.dtd}owner').text = channel_info['owner']
        ...
        # iTunes explicit
        ET.SubElement(channel, '{http://www.itunes.com/dtds/podcast-1.0.dtd}explicit').text = channel_info['explicit']
    ...
    def _add_episode_to_rss(self, channel: ET.Element, episode_info: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            description_text = sanitize_xml_content(episode_info['description'][:self.config.MAX_XML_CONTENT_LENGTH])
            ...
            # iTunes-specific tags
            ET.SubElement(item, '{http://www.itunes.com/dtds/podcast-1.0.dtd}title').text = title_text
            ET.SubElement(item, '{http://www.itunes.com/dtds/podcast-1.0.dtd}summary').text = description_text
            ET.SubElement(item, '{http://www.itunes.com/dtds/podcast-1.0.dtd}duration').text = str(episode_info['duration'])
            ET.SubElement(item, '{http://www.itunes.com/dtds/podcast-1.0.dtd}keywords').text = episode_info['keywords']
