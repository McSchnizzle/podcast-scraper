...
class SecurityValidator:
    ...
    def validate_filename(self, filename: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            is_valid = (
                len(filename) > 0
                and len(filename) <= self.max_filename_length
                and not any(
                    re.search(pattern, filename, re.IGNORECASE)
                    for pattern in self.blocked_patterns
                )
                and file_ext in self.allowed_file_extensions
            )
    ...
    def validate_content(
        ...
        try:
            ...
            if original_length > self.max_content_length:
                ...
                logger.warning(
                    f"⚠️ Content truncated from {original_length} to {len(content)} chars"
                )
    ...
    def validate_database_input(self, value: Any) -> Tuple[bool, Any]:
        ...
        if isinstance(value, str):
            ...
            if suspicious_patterns:
                logger.warning(
                    f"⚠️ Suspicious SQL patterns in input: {suspicious_patterns}"
                )
                return False, value
    ...
    def validate_resource_limits(self, **kwargs) -> Dict[str, Any]:
        ...
        results = {"within_limits": True, "violations": [], "warnings": []}

        # File size limits
        file_size = kwargs.get("file_size", 0)
        if file_size > 100 * 1024 * 1024:  # 100MB
            results["violations"].append(
                f"File size too large: {file_size / 1024 / 1024:.1f}MB"
            )
        ...
        if processing_time > 300:  # 5 minutes
            results["violations"].append(
                f"Processing time too long: {processing_time}s"
            )
            results["within_limits"] = False
        ...
        if requests_per_minute > 60:
            results["violations"].append(
                f"Request rate too high: {requests_per_minute}/min"
            )
            results["within_limits"] = False
    ...
    def create_security_report(
        ...
        critical_issues = [
            ...
            if not v.get("valid", False) and v.get("critical", False)
        ]
        warnings = [
            v
            for v in validations
            if not v.get("valid", False) and not v.get("critical", False)
        ]
        ...
        return {
            ...
            "status": (
                ...
                else "AT_RISK" if security_score >= 0.8 else "VULNERABLE"
            ),
    ...
    def _generate_security_recommendations(
        self, critical_issues: List[Dict], warnings: List[Dict]
    ) -> List[str]:
        ...
        if critical_issues:
            ...
            if "xml_injection" in issue_types:
                recommendations.append(
                    "• Enable comprehensive XML content sanitization"
                )
        ...
        if not critical_issues and not warnings:
            recommendations.append(
                "✅ Security validation passed - maintain current security practices"
            )
