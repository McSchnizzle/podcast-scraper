...
def extract_openai_settings_keys(config_obj) -> Set[str]:
    ...
    if hasattr(config_obj, "OPENAI_SETTINGS") and isinstance(
        config_obj.OPENAI_SETTINGS, dict
    ):
...
def validate_critical_config(warn_only: bool = False) -> bool:
    ...
    critical_dirs = ["daily_digests", "transcripts", "audio_cache"]
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Configuration validation for podcast scraper"
    )
    parser.add_argument(
        "--all",
        action="store_true",
