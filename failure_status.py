    ...
    for db_name, db_path in [
        ...
        try:
            ...
            if failure_stats.get("total_failed_episodes", 0) > 0:
                ...
                top_failures = list(failure_stats.get("failure_reasons", {}).items())[
                    :3
                ]
                if top_failures:
            ...
            if retry_candidates:
                ...
                for candidate in retry_candidates[:3]:  # Show first 3
                    print(
                        f"    - {candidate['episode_id']}: {candidate['title']} (category: {candidate['failure_category']}, attempt #{candidate['retry_count'] + 1})"
                    )
