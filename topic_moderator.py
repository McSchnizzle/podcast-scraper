...
class TopicModerator:
                ...
                for row in cursor.fetchall():
                    ...
                    try:
                        ...
                        for topic in [
                            ...
                            if topic in scores and isinstance(
                                scores[topic], (int, float)
                            ):
                        ...
                        logger.warning(
                            f"Error parsing scores for episode {episode_id}: {e}"
                        )
                        continue
    ...
    def display_episodes_by_topic(self, episodes_data: Dict[str, List[Dict]]):
        ...
        print(
            f"{total_qualifying} episodes qualify for digest (≥{self.RELEVANCE_THRESHOLD} relevance)"
        )
        ...
        for topic in [
            ...
            for i, episode in enumerate(episodes, 1):
                ...
                other_display = (
                    f" (also: {', '.join(other_topics)})" if other_topics else ""
                )

                print(
                    f"{i:2}. {status_icon} {score_display} \"{episode['title'][:50]}{'...' if len(episode['title']) > 50 else ''}\""
                )
                print(
                    f"     {episode['db_type']} | {episode['feed_title'] or 'Unknown Feed'}{other_display}"
                )
    ...
    def reassign_episode_topic(
        self, episode_id: str, new_topic: str, db_type: str = None
    ) -> bool:
        """
        ...
        for db_name, db_path in databases_to_try:
            ...
            try:
                ...
                cursor.execute(
                    "SELECT id, title, topic_relevance_json FROM episodes WHERE episode_id = ?",
                    (episode_id,),
                )
                ...
                if result:
                    ...
                    if new_topic == "EXCLUDE":
                        cursor.execute(
                            "UPDATE episodes SET digest_topic = NULL WHERE id = ?",
                            (db_id,),
                        )
                        print(f'✅ Episode excluded from digest: "{title[:50]}"')
                    else:
                        cursor.execute(
                            "UPDATE episodes SET digest_topic = ? WHERE id = ?",
                            (new_topic, db_id),
                        )
        ...
        for db_name, db_path in self.databases.items():
            ...
            try:
                        ...
                        try:
                            ...
                            for t in [
                                ...
                                if t in scores and isinstance(scores[t], (int, float)):
                                    if (
                                        scores[t] > best_score
                                        and scores[t] >= self.RELEVANCE_THRESHOLD
                                    ):
                                        best_topic = t
                                        best_score = scores[t]
                ...
                if changes > 0:
                    ...
                    print(
                        f"✅ Approved {changes} episodes{topic_str} in {db_name} database"
                    )
                    ...
                    if scores_json:
                        try:
                            ...
                            for topic, score in topic_scores:
                                status_icon = (
                                    "✅" if score >= self.RELEVANCE_THRESHOLD else "❌"
                                )
...
def main():
    parser = argparse.ArgumentParser(
        description="Topic Moderator - Review and manage episode topic assignments"
    )
    parser.add_argument(
        "--review", action="store_true", help="Review episodes pending digest"
    )
    parser.add_argument(
        "--reassign",
        nargs=2,
        metavar=("EPISODE_ID", "TOPIC"),
    ...
    parser.add_argument(
        "--details", metavar="EPISODE_ID", help="Show detailed episode information"
    )
    parser.add_argument(
        "--days",
        type=int,
        default=7,
        help="Days back to look for episodes (default: 7)",
    )
    ...
    if args.review:
        ...
        if args.approve == "all":
            ...
            print(
                f"✅ Approved {approved} {args.approve} episodes for digest generation"
            )
        ...
        print("\nUsage:")
        print(
            "  python topic_moderator.py --review           # Review pending episodes"
        )
        print(
            "  python topic_moderator.py --approve          # Approve all qualifying episodes"
        )
        print(
            "  python topic_moderator.py --approve Tech     # Approve Technology episodes only"
        )
