...
class MusicIntegrator:
    ...
    def generate_master_music(self, topic: str, duration: int = 60) -> Optional[str]:
        ...
        if not self.elevenlabs_api_key:
            logger.warning(
                "ElevenLabs API key not available - skipping music generation"
            )
            return None

        topic_config = self.topics_config.get(topic, {})
        music_prompt = topic_config.get(
            "music_prompt", f"Background music suitable for {topic} content"
        )
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.info(
                    f"✅ Generated master music: {cache_filename} ({len(response.content)} bytes)"
                )
                return str(cache_path)
            else:
                logger.error(
                    f"❌ Music generation failed: {response.status_code} - {response.text}"
                )
                return None
    ...
    def extract_music_segment(
        ...
        try:
            ...
            if result.returncode == 0:
                logger.info(
                    f"✅ Extracted {duration_seconds}s segment starting at {start_seconds}s"
                )
                return True
            else:
    ...
    def generate_music(
        self, topic: str, music_type: str = "background"
    ) -> Optional[str]:
        ...
        success = self.extract_music_segment(
            master_music, config["start"], config["duration"], str(segment_path)
        )
    ...
    def create_static_music_files(self):
        ...
        static_files = {
            "intro": {
                "duration": 5,
                "freq": "440,554,659",
                "description": "Major chord intro",
            },
            "outro": {
                "duration": 3,
                "freq": "659,554,440",
                "description": "Descending chord outro",
            },
            "background": {
                "duration": 60,
                "freq": "220",
                "description": "Low ambient tone",
            },
        }
            ...
            if not cache_path.exists():
                try:
                    ...
                    if config["freq"] == "440,554,659":  # Major chord intro
                        sine_filter = (
                            f'sine=frequency=440:duration={config["duration"]}'
                        )
                    ...
                    if result.returncode == 0:
                        logger.info(
                            f"✅ Created static {music_type} music ({config['description']})"
                        )
                    else:
                        logger.error(
                            f"❌ Failed to create static {music_type} music: {result.stderr}"
                        )
    ...
    def mix_audio_with_music(
        self, audio_file: str, topic: str, output_file: str
    ) -> bool:
        ...
        try:
            ...
            cmd = [
                "ffmpeg",
                "-i",
                intro_music,  # Input 0: Intro music (8 seconds)
                "-i",
                audio_file,  # Input 1: Main narration
                "-i",
                outro_music,  # Input 2: Outro music (8 seconds)
                "-filter_complex",
                filter_complex,
                "-map",
                "[final]",
            ...
            if result.returncode == 0:
                logger.info(
                    f"✅ Audio enhanced with crossfade transitions: {output_file}"
                )
                return True
            else:
                ...
                if fallback_result.returncode == 0:
                    ...
                    logger.info(
                        f"📄 Final fallback: copied original audio to {output_file}"
                    )
                    return False
    ...
    def add_segment_transition(
        self, topic: str, transition_type: str = "soft"
    ) -> Optional[str]:
        ...
        try:
            ...
            if success:
                logger.info(
                    f"✅ Generated {transition_type} transition: {cache_filename}"
                )
                return str(cache_path)
            else:
                return None
