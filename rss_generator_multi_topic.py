...
class MultiTopicRSSGenerator:
    ...
    def _process_deployment_metadata(self, metadata: Dict, days: int) -> List[Dict]:
        ...
        for episode in metadata.get("episodes", []):
            try:
                ...
                digest_files.append(
                    {
                        ...
                        "date": file_date,
                        "md_file": (
                            Path(episode["local_path"]).with_suffix(".md")
                            if Path(episode["local_path"]).exists()
                            else None
                        ),
                ...
                print(
                    f"⚠️ Error processing episode {episode.get('file_key', 'unknown')}: {e}"
                )
    ...
    def _find_digest_files_fallback(self, days=7) -> List[Dict]:
        ...
        for md_file in digest_dir.glob("*_digest_*.md"):
            ...
            if not mp3_file.exists():
                ...
                if not mp3_file.exists():
                    ...
                    mp3_file = (
                        digest_dir
                        / f"complete_topic_digest_{timestamp_str}_enhanced.mp3"
                    )
                    if not mp3_file.exists():
                        mp3_file = (
                            digest_dir / f"complete_topic_digest_{timestamp_str}.mp3"
                        )
                        if not mp3_file.exists():
                            continue
        ...
        for mp3_file in digest_dir.glob("*.mp3"):
            ...
            if "_enhanced" not in mp3_file.name:
                ...
                enhanced_path = mp3_file.parent / (
                    mp3_file.stem + "_enhanced" + mp3_file.suffix
                )
    ...
    def _generate_episode_description(self, topic: str, date: datetime) -> str:
        ...
        base_description = topic_info.get(
            "description", "Covering the latest developments and insights."
        )
    ...
    def _validate_mp3_file(self, digest_info: Dict) -> bool:
        ...
        try:
            ...
            if (
                ...
                print(
                    f"⚠️ SKIP size mismatch: {mp3_file} (expected {expected_size}, got {file_size})"
                )
                return False
            ...
            print(
                f"⚠️ Error validating MP3 {digest_info.get('mp3_file', 'unknown')}: {e}"
            )
            return False
    ...
    def generate_rss(self, output_file="daily-digest.xml", max_items=100) -> bool:
        ...
        try:
            ...
            today_mp3s = [
                ...
                if d["date"].date() == today and self._validate_mp3_file(d)
            ]
            ...
            if len(digest_files) > max_items:
                ...
                print(
                    f"📡 Applied RSS item cap: showing {max_items} of {len(digest_files)} available episodes"
                )
            ...
            # Podcast metadata
            SubElement(channel, "title").text = sanitize_xml_content(
                self.podcast_info["title"]
            )
            SubElement(channel, "description").text = sanitize_xml_content(
                self.podcast_info["description"]
            )
            ...
            SubElement(channel, "managingEditor").text = (
                f"{self.podcast_info['email']} ({self.podcast_info['author']})"
            )
            SubElement(channel, "webMaster").text = (
                f"{self.podcast_info['email']} ({self.podcast_info['author']})"
            )
            SubElement(channel, "lastBuildDate").text = now_utc().strftime(
                "%a, %d %b %Y %H:%M:%S %z"
            )

            # iTunes-specific tags
            SubElement(channel, "itunes:author").text = sanitize_xml_content(
                self.podcast_info["author"]
            )
            SubElement(channel, "itunes:summary").text = sanitize_xml_content(
                self.podcast_info["description"]
            )
            ...
            for digest_info in digest_files:
                ...
                # Basic item info
                SubElement(item, "title").text = sanitize_xml_content(
                    digest_info["title"]
                )
                SubElement(item, "description").text = sanitize_xml_content(
                    digest_info["description"]
                )

                # Generate permalink and stable GUID
                permalink = f"{self.podcast_info['website']}/{digest_info['timestamp']}"
                stable_guid = self._generate_stable_guid(
                    digest_info["topic"], digest_info["timestamp"], digest_info["date"]
                )
                ...
                # iTunes episode info
                SubElement(item, "itunes:title").text = sanitize_xml_content(
                    digest_info["title"]
                )
                SubElement(item, "itunes:summary").text = sanitize_xml_content(
                    digest_info["description"]
                )
                SubElement(item, "itunes:duration").text = str(
                    self._estimate_duration(file_size)
                )
            ...
            print(
                f"✅ RSS feed generated: {output_file} with {items_added} valid items"
            )
            ...
            try:
                ...
                if stats.get("total_summaries", 0) > 0:
                    print(
                        f"📊 Episode summaries: {stats['total_summaries']} cached, {stats['unique_topics']} topics"
                    )
