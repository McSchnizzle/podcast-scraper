...
class Config:
    ...
    def __init__(self, require_claude: bool = True):
        ...
        # Database settings
        self.DB_SETTINGS = {"timeout": 30, "check_same_thread": False}  # seconds
        ...
        self.FEED_SETTINGS = {
            ...
            "lookback_hours": max(
                1, min(168, int(os.getenv("FEED_LOOKBACK_HOURS", "48")))
            ),  # 1-168 hours (7 days max)
            "grace_minutes": int(
                os.getenv("FEED_GRACE_MINUTES", "15")
            ),  # Avoid boundary flapping
            "max_feed_bytes": int(
                os.getenv("MAX_FEED_BYTES", "5242880")
            ),  # 5MB safety cap
            "politeness_delay_ms": int(
                os.getenv("FETCH_POLITENESS_MS", "250")
            ),  # Polite delay between requests
            "enable_http_caching": os.getenv("ENABLE_HTTP_CACHING", "1")
            == "1",  # ETag/Last-Modified support
            "detect_feed_order": os.getenv("DETECT_FEED_ORDER", "1")
            == "1",  # Auto-detect typical_order
            "item_seen_retention_days": int(
                os.getenv("ITEM_SEEN_RETENTION_DAYS", "30")
        ...
        self.REASONING_EFFORT = {
            ...
            ),  # Higher fidelity for validation
        }
    ...
    def _validate_environment(self, require_claude: bool = True):
        ...
        if not self.GITHUB_TOKEN:
            warnings.append(
                "GITHUB_TOKEN not set - deployment features will be disabled"
            )

        if not self.ELEVENLABS_API_KEY:
            warnings.append(
                "ELEVENLABS_API_KEY not set - TTS features will be disabled"
            )

        if not self.OPENAI_API_KEY:
            warnings.append(
                "OPENAI_API_KEY not set - OpenAI digest generation will be disabled"
            )
        ...
        if missing_fields:
            errors.append(
                f"Missing OPENAI_SETTINGS fields: {', '.join(missing_fields)}"
            )
        else:
        ...
        if require_claude:
            ...
            try:
                ...
                warnings.append(
                    "Claude Code CLI not found - digest generation will use OpenAI fallback"
                )
        ...
        if errors:
            logger.error(
                "❌ Critical dependencies missing - some features may not work"
            )
        else:
    ...
    def sync_feeds_to_database(
        self, db_path: str = "podcast_monitor.db", force_update: bool = False
    ):
        ...
        if db_feed_count == 0 or force_update:
            ...
            logger.info(
                f"Database has {db_feed_count} feeds - using database as source of truth"
            )
    ...
    def remove_feed_from_db(
        self, feed_id: int, db_path: str = "podcast_monitor.db"
    ) -> bool:
    ...
    def get_next_status(
        self, current_status: str, episode_type: str = "rss"
    ) -> Optional[str]:
