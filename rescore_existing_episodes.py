    ...
    try:
        ...
        for episode_id, transcript_path in episodes:
            ...
            if not Path(transcript_path).exists():
                logger.warning(
                    f"Transcript not found for {episode_id}: {transcript_path}"
                )
                continue
...
def main():
    ...
    logger.info(
        "🔄 Starting re-scoring of existing episodes with new 4-topic structure"
    )
    ...
    logger.info(
        f"🎯 Re-scoring complete - updated {total_scored} total episodes with new 4-topic structure"
    )
