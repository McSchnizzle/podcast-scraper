...
class DatabaseConnectionFactory:
    ...
    @classmethod
    def get_connection(cls, db_path: str, readonly: bool = False, 
                      validate_schema: bool = True) -> sqlite3.Connection:
        """
        ...
        with cls._lock:
            ...
            try:
                ...
                logger.debug(f"Database connection established: {db_path} (readonly={readonly})")
                return conn
    ...
    @classmethod
    def _configure_connection(cls, conn: sqlite3.Connection, readonly: bool = False) -> None:
        """Configure connection with enforced standards"""
        ...
        if readonly:
            conn.execute("PRAGMA query_only = ON")  # Prevent writes on readonly connections
        
        # Verify foreign keys are actually enabled
        fk_status = conn.execute("PRAGMA foreign_keys").fetchone()[0]
        if fk_status != 1:
            raise sqlite3.Error(f"Failed to enable foreign key enforcement (status: {fk_status})")
    ...
    def _validate_schema_version(cls, conn: sqlite3.Connection, db_path: str) -> None:
        ...
        try:
            ...
            if current_version == 0:
                ...
                logger.info(f"Database {db_path} has no schema version (user_version=0), skipping validation")
                return
...
@contextmanager
def get_db_connection(db_path: str, readonly: bool = False, validate_schema: bool = True):
    """
...
def get_connection(db_path: str, readonly: bool = False, validate_schema: bool = True) -> sqlite3.Connection:
    """
...
def execute_query(db_path: str, query: str, params: tuple = (), readonly: bool = False) -> list:
    """
...
def sqlite3_connect_deprecated(db_path: str, **kwargs):
    ...
    """
    logger.warning(f"DEPRECATED: sqlite3_connect_deprecated() called from {db_path}. Use get_connection() instead.")
