...
def validate_response_safety(text: str, max_prompt_echo: int = 100) -> bool:
    ...
    for indicator in INJECTION_INDICATORS:
        if indicator in text_lower:
            logger.warning(
                f"🚨 Potential injection detected: '{indicator}' found in response"
            )
            return False
    ...
    if len(long_echo_lines) > 2:  # Allow some long lines but flag excessive echoing
        logger.warning(
            f"🚨 Excessive prompt echoing detected: {len(long_echo_lines)} long lines"
        )
        return False
...
def call_openai_with_backoff(
    ...
    **kwargs,
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            logger.debug(
                f"OpenAI call attempt {attempt}/{MAX_RETRIES}: component={component} run={run_id}"
            )
            ...
            if validate_safety and not validate_response_safety(raw_text):
                raise ValueError(
                    "Response failed safety validation - potential injection attempt"
                )
            ...
            if attempt == MAX_RETRIES:
                ...
                raise RuntimeError(
                    f"OpenAI call failed after {MAX_RETRIES} attempts: {last_exception}"
                )
...
def _persist_raw_response(
    run_id: str, component: str, response: Any, raw_text: str, metadata: Dict[str, Any]
) -> None:
