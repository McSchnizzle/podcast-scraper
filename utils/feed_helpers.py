    ...
    if result:
        ...
        try:
            if first_seen_str.endswith("Z"):
                first_seen = datetime.fromisoformat(first_seen_str[:-1]).replace(
                    tzinfo=None
                )
            else:
...
def detect_typical_order(entries: List[Any]) -> str:
    ...
    reverse_chrono = all(
        dated_entries[i] >= dated_entries[i + 1] for i in range(len(dated_entries) - 1)
    )
...
def get_effective_lookback_hours(
    cursor: sqlite3.Cursor, feed_id: int, default_hours: int
) -> int:
    """
...
def should_suppress_warning(
    cursor: sqlite3.Cursor, feed_id: int, warning_type: str, suppress_hours: int = 24
) -> bool:
    """
...
def update_warning_timestamp(cursor: sqlite3.Cursor, feed_id: int, warning_type: str):
    """
...
def get_feed_cache_headers(
    cursor: sqlite3.Cursor, feed_id: int
...
def extract_http_cache_headers(
...
def cleanup_old_item_seen_records(cursor: sqlite3.Cursor, retention_days: int):
    ...
    cursor.execute("DELETE FROM item_seen WHERE last_seen_utc < ?", (cutoff_str,))
