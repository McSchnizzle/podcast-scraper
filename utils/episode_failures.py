...
class FailureManager:
    ...
    def log_episode_failure(
        ...
        try:
            ...
            if not result:
                self.logger.warning(
                    f"Episode {episode_id} not found for failure logging"
                )
                return False
            ...
            max_retries = self.FAILURE_CATEGORIES.get(failure_category, {}).get(
                "max_retries", self.MAX_RETRY_ATTEMPTS
            )
            final_status = (
                "failed" if new_retry_count >= max_retries else current_status
            )
            ...
            self._log_detailed_failure(
                cursor, db_id, failure_reason, failure_category, traceback_info
            )
            ...
            status_msg = (
                ...
                else f"retry #{new_retry_count}"
            )
            self.logger.info(
                f"📝 Episode {episode_id} {status_msg}: [{failure_category}] {failure_reason}"
            )
    ...
    def _log_detailed_failure(
        ...
        try:
            ...
            if "no such column: episode_pk" in str(e):
                ...
                cursor.execute(
                    "SELECT episode_id FROM episodes WHERE id = ?", (episode_pk,)
                )
                result = cursor.fetchone()
                if result:
                    episode_id = result[0]
                    ...
                    raise RuntimeError(
                        f"Cannot find episode with id {episode_pk} for failure logging"
                    )
            else:
                raise
    ...
    def _create_failures_table(self, cursor):
        ...
        self.logger.info(
            "✅ Created episode_failures table with Option A schema and indexes"
        )
    ...
    def get_retry_candidates(self) -> List[Dict]:
        ...
        try:
            ...
            for row in cursor.fetchall():
                ...
                max_retries = category_config.get(
                    "max_retries", self.MAX_RETRY_ATTEMPTS
                )
            ...
            self.logger.info(
                f"🔄 Found {len(retry_candidates)} episodes eligible for retry"
            )
    ...
    def _extract_failure_category(self, failure_reason: str) -> str:
        ...
        if any(
            ...
            for term in ["download", "fetch", "network", "connection"]
        ):
            return "download"
        elif any(
            term in reason_lower for term in ["transcrib", "audio", "speech", "whisper"]
        ):
            return "transcription"
        elif any(
            term in reason_lower for term in ["score", "openai", "api", "rate limit"]
        ):
            return "scoring"
        elif any(term in reason_lower for term in ["digest", "claude", "generation"]):
            return "digest"
        elif any(
            term in reason_lower for term in ["deploy", "github", "rss", "publish"]
        ):
            return "publishing"
        else:
            return "general"

    def mark_episode_recovered(
        self, episode_id: str, recovery_notes: str = None
    ) -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"✅ Episode {episode_id} marked as recovered: {recovery_notes}"
            )
            return True
...
class RetryProcessor:
    ...
    def process_retry_queue(self, max_retries_per_run: int = 5) -> Dict:
        ...
        for candidate in candidates_to_process:
            self.logger.info(
                f"🔄 Retrying {candidate['episode_id']}: {candidate['title']}"
            )
    ...
    def _retry_episode(self, candidate: Dict) -> bool:
        ...
        try:
            ...
            if failure_category == "download" or current_status == "pre-download":
                ...
                self.logger.warning(
                    f"Unknown retry scenario for {episode_id}: {failure_category}/{current_status}"
                )
                return False

        except Exception as e:
            self.logger.error(
                f"Exception during retry for {candidate['episode_id']}: {e}"
            )
    ...
    def _retry_download_and_transcription(self, processor, candidate: Dict) -> bool:
        ...
        try:
            ...
            if not audio_file:
                self.failure_manager.log_episode_failure(
                    episode_id, "Audio download failed on retry", "download"
                )
                return False
            ...
            if not transcript_path:
                self.failure_manager.log_episode_failure(
                    episode_id, "Transcription failed on retry", "transcription"
                )
                return False
            ...
            self.logger.info(
                f"✅ Successfully retried download+transcription for {episode_id}"
            )
            return True

        except Exception as e:
            self.failure_manager.log_episode_failure(
                candidate["episode_id"], f"Download retry failed: {str(e)}", "download"
            )
            return False
    ...
    def _retry_transcription_only(self, processor, candidate: Dict) -> bool:
        ...
        try:
            ...
            if not transcript_path:
                self.failure_manager.log_episode_failure(
                    episode_id, "Transcription retry failed", "transcription"
                )
                return False
            ...
            self.failure_manager.log_episode_failure(
                candidate["episode_id"],
                f"Transcription retry failed: {str(e)}",
                "transcription",
            )
            return False
    ...
    def _retry_scoring(self, candidate: Dict) -> bool:
        ...
        try:
            ...
            success = scorer.score_pending_in_db(
                self.db_path, max_episodes=1, target_episode_id=episode_id
            )
            ...
            if success:
                ...
                self.failure_manager.log_episode_failure(
                    episode_id, "Scoring retry failed", "scoring"
                )
                return False

        except Exception as e:
            self.failure_manager.log_episode_failure(
                candidate["episode_id"], f"Scoring retry failed: {str(e)}", "scoring"
            )
            return False
