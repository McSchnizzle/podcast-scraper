...
from utils.db import get_connection, EXPECTED_SCHEMA_VERSION
...
class StartupValidator:
    ...
    def run_all_checks(self, strict: bool = True) -> bool:
        ...
        try:
            ...
            if has_errors:
                ...
                logger.error(f"❌ Startup validation failed in strict mode with {len(self.warnings)} warnings")
                for warning in self.warnings:
                    logger.error(f"  WARNING: {warning}")
                return False
            elif has_warnings:
                logger.warning(f"⚠️ Startup validation passed with {len(self.warnings)} warnings")
                for warning in self.warnings:
    ...
    def _check_database_health(self, db_path: str) -> None:
        ...
        try:
            ...
            with get_connection(db_path, readonly=True, validate_schema=False) as conn:
                ...
                if fk_status != 1:
                    self.errors.append(f"Foreign keys not enabled in {db_path} (status: {fk_status})")
                else:
                ...
                if current_version == 0:
                    self.warnings.append(f"Schema version not set in {db_path} (user_version=0)")
                    ...
                    logger.debug(f"  ✅ Schema version correct: {db_path} v{current_version}")
                
                # Quick foreign key integrity check
                cursor.execute("PRAGMA foreign_key_check")
                violations = cursor.fetchall()
                if violations:
                    self.errors.append(f"Foreign key violations in {db_path}: {len(violations)} violations")
                else:
                ...
                if missing_tables:
                    self.errors.append(f"Critical tables missing in {db_path}: {missing_tables}")
                else:
    ...
    def _check_environment_variables(self) -> None:
        ...
        if missing_critical:
            self.errors.append(f"Critical environment variables missing: {missing_critical}")
        
        if missing_important:
            self.warnings.append(f"Important environment variables missing: {missing_important}")
...
def validate_startup(databases: Optional[List[str]] = None, strict: bool = True) -> bool:
    """
