    ...
    def run_preflight_checks(cls, db_paths: Optional[list] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            for db_path in db_paths:
                ...
                logger.info(f"     - Foreign Keys: {'ON' if db_info.get('foreign_keys') == 1 else 'OFF'}")
                logger.info(f"     - Journal Mode: {db_info.get('journal_mode', 'UNKNOWN')}")
                logger.info(f"     - Synchronous: {db_info.get('synchronous', 'UNKNOWN')}")
                logger.info(f"     - Schema Version: {db_info.get('user_version', 'UNKNOWN')} (expected: {EXPECTED_SCHEMA_VERSION})")
            ...
            logger.info(f"   Python Version: {results['system_info']['python_version']}")
