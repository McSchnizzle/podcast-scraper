...
SENSITIVE_PATTERNS = [
    # API Keys
    (r'(?i)(api[_-]?key|apikey|key)[\'"\s]*[:=]\s*[\'"]([a-zA-Z0-9\-_]{20,})[\'"]', r'\1: "[REDACTED_API_KEY]"'),
    ...
    # URLs with secrets
    (r'(https?://[^/\s]*?)([?&](api_key|token|key|secret|password)=)([^&\s]+)', r'\1\2[REDACTED_PARAM]'),
    
    # Generic tokens
    (r'(?i)(token|secret|password)[\'"\s]*[:=]\s*[\'"]([a-zA-Z0-9\-_]{16,})[\'"]', r'\1: "[REDACTED_SECRET]"'),
    
    # Base64 encoded secrets (common pattern)
    (r'(?i)(authorization|auth)[\'"\s]*[:=]\s*[\'"]([a-zA-Z0-9+/=]{32,})[\'"]', r'\1: "[REDACTED_AUTH]"'),
    
    # GitHub tokens (ghp_, gho_, ghs_, ghr_)
    (r'gh[psor]_[a-zA-Z0-9]{36,}', '[REDACTED_GITHUB_TOKEN]'),
    
    # Generic long alphanumeric strings that might be secrets
    (r'(?i)(key|token|secret|password)["\']\s*:\s*["\']([a-zA-Z0-9\-_]{32,})["\']', r'\1": "[REDACTED_CREDENTIAL]"'),
]
...
def redact_dict(data: dict) -> dict:
    ...
    for key, value in data.items():
        ...
        if any(sensitive in key_lower for sensitive in ['key', 'token', 'secret', 'password', 'auth', 'bearer']):
            redacted[key] = '[REDACTED]'
        elif isinstance(value, dict):
            redacted[key] = redact_dict(value)
        elif isinstance(value, list):
            redacted[key] = [redact_dict(item) if isinstance(item, dict) else redact_secrets(str(item)) for item in value]
