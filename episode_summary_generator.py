...
class EpisodeSummaryGenerator:
    ...
    def create_chunks(self, content: str, episode_id: str, title: str = "") -> List[Tuple[int, int, int, str]]:
        """
        ...
        while start < len(content):
            ...
            if chunk_text:
                ...
                logger.info(f"Chunk {chunk_index + 1}: {start}-{end} • \"{title or episode_id}\"")
                chunk_index += 1
    ...
    def _generate_fallback_chunk_summary(self, episode_id: str, chunk_index: int, content: str, topic: str) -> Dict:
    ...
    def _generate_mock_chunk_summary(self, episode_id: str, chunk_index: int, content: str, topic: str) -> Dict:
    ...
    def generate_chunk_summary(
        ...
        if not self.feature_enabled:
            ...
            return self._generate_fallback_chunk_summary(episode_id, chunk_index, content, topic)
        ...
        # Mock mode
        if self.mock_mode:
            return self._generate_mock_chunk_summary(episode_id, chunk_index, content, topic)
        
        # Check cache (idempotency)
        cached_result = self._get_cached_chunk_summary(episode_id, chunk_index)
        if cached_result:
            logger.info(f"📖 Using cached chunk summary: {episode_id} chunk {chunk_index}")
            return cached_result
        
        if len(content.strip()) < 50:
            logger.info(f"Chunk {chunk_index} too short ({len(content)} chars), skipping")
            return None
...
Provide a structured summary with the essential information from this segment."""
            ...
            logger.info(f"🤖 Generating chunk summary: {episode_id} chunk {chunk_index} ({len(clean_content)} chars)")
            
            idempotency_key = generate_idempotency_key(episode_id, chunk_index, self.model, "1.0")
            ...
            logger.info(f"✅ Generated chunk summary: {episode_id} chunk {chunk_index} ({result.metadata.get('tokens_out', 0)} tokens)")
            return summary_data
            
        except Exception as e:
            logger.error(f"❌ Failed to generate chunk summary for {episode_id} chunk {chunk_index}: {e}")
            return self._generate_fallback_chunk_summary(episode_id, chunk_index, content, topic)
