...
# Import utilities
from utils.datetime_utils import cutoff_utc, now_utc, parse_entry_to_utc, to_utc
from utils.db import get_connection
from utils.feed_helpers import (
    cleanup_old_item_seen_records,
    compute_cutoff_with_grace,
    content_hash,
    detect_typical_order,
    ensure_feed_metadata_exists,
    extract_http_cache_headers,
    format_feed_stats,
    get_effective_lookback_hours,
    get_feed_cache_headers,
    get_or_set_first_seen,
    handle_conditional_get,
    item_identity_hash,
    should_suppress_warning,
    update_feed_cache_headers,
...
class FeedMonitor:
    ...
    def add_youtube_channel(
        self, channel_url, topic_category, title=None, channel_id=None
    ):
        ...
        try:
            ...
            rss_url = (
                f"https://www.youtube.com/feeds/videos.xml?channel_id={extracted_id}"
            )
    ...
    def check_new_episodes(self, hours_back=None, feed_types=None):
        ...
        logger.info(
            f"🕐 Feed monitoring started: global_lookback={global_lookback}h grace={grace_minutes}m caching={enable_http_caching}"
        )
        ...
        try:
            ...
            cleanup_old_item_seen_records(
                cursor, self.feed_settings["item_seen_retention_days"]
            )
            ...
            for feed_id, url, title, feed_type, topic_category in feeds:
                ...
                try:
                    ...
                    effective_lookback = get_effective_lookback_hours(
                        cursor, feed_id, global_lookback
                    )
                    cutoff_time = compute_cutoff_with_grace(
                        effective_lookback, grace_minutes
                    )
    ...
    def _get_feeds_to_process(
        self, cursor: sqlite3.Cursor, feed_types: Optional[List[str]]
    ) -> List[Tuple]:
        ...
        if feed_types:
            ...
            cursor.execute(
                "SELECT id, url, title, type, topic_category FROM feeds WHERE active = 1 ORDER BY id"
            )
        ...
        try:
            ...
            if enable_caching:
                ...
                if cached:
                    ...
                    logger.info(
                        f"📦 {title} items=0 dated=0 nodate=0 cutoff={cutoff_time.strftime('%Y-%m-%dT%H:%M:%S')}Z lookback={lookback_hours}h etag_hit=true"
                    )
                    logger.info(
                        f"   {format_feed_stats(dict(stats, duration_ms=duration_ms))}"
                    )

                    # Update last checked time
                    cursor.execute(
                        'UPDATE feeds SET last_checked = datetime("now", "UTC") WHERE id = ?',
                        (feed_id,),
                    )
                ...
                update_feed_cache_headers(
                    cursor, feed_id, new_headers["etag"], new_headers["last_modified"]
                )

                # Check response size
                if len(response.content) > self.feed_settings["max_feed_bytes"]:
                    raise ValueError(
                        f"Feed too large: {len(response.content)} bytes > {self.feed_settings['max_feed_bytes']}"
                    )
    ...
    def _process_feed_entries(
        ...
        entries_to_process = (
            feed_data.entries[:max_items] if max_items > 0 else feed_data.entries
        )
        ...
        for entry in entries_to_process:
            try:
                ...
                item_hash = item_identity_hash(
                    entry_guid, entry_link, entry_title, enclosure_url
                )
                ...
                if not pub_date:
                    ...
                    stats["nodate"] += 1
                    logger.debug(
                        f"SYNTHETIC DATE: {title} :: {entry_title} -> {pub_date.isoformat()}Z"
                    )
                else:
                ...
                if pub_date < cutoff_time:
                    stats["skipped"] += 1
                    logger.debug(
                        f"SKIP OLD: {title} :: {entry_title} ({pub_date.isoformat()})"
                    )
                    continue

                # Check for duplicates using enhanced detection
                if self._is_enhanced_duplicate(
                    cursor, feed_id, item_hash, entry_title, pub_date
                ):
                    stats["duplicate"] += 1
                ...
                episode_id = hashlib.md5(
                    (entry_guid or entry_link or entry_title).encode()
                ).hexdigest()[:8]
    ...
    def _check_feed_warnings(
        ...
        if entries and stats["nodate"] == 0:
            ...
            if newest_dates:
                ...
                stale_threshold = now_utc() - timedelta(
                    days=self.feed_settings["stale_feed_days"]
                )
        ...
        if len(entries) > 0 and stats["nodate"] == len(entries):
            if not should_suppress_warning(cursor, feed_id, "no_dates", 24):
                logger.warning(
                    f"⚠️ NO DATES: {title} - all {len(entries)} entries lack dates"
                )
    ...
    def _emit_run_telemetry(self, stats: Dict[str, Any], duration_ms: int):
        ...
        try:
            ...
            telemetry.record_metric(
                "ingest.feeds.total.count", stats["total_feeds"], run_id=run_id
            )
            telemetry.record_metric(
                "ingest.feeds.processed.count", stats["processed_feeds"], run_id=run_id
            )
            telemetry.record_metric(
                "ingest.feeds.errors.count", stats["errors"], run_id=run_id
            )
            telemetry.record_metric(
                "ingest.episodes.new.count", stats["new_episodes"], run_id=run_id
            )
            telemetry.record_metric(
                "ingest.http.cache_hits.count", stats["http_cache_hits"], run_id=run_id
            )

            # Emit duration
            telemetry.record_metric(
                "ingest.run.duration.ms", duration_ms, run_id=run_id
            )
    ...
    def _log_feed_error(self, title: str, feed_type: str, error: str):
        ...
        logger.error(f'FEED_ERROR feed="{title}" type={feed_type} error="{error}"')

    def _is_duplicate_episode(
        self, cursor, episode_id: str, audio_url: str, title: str, pub_date: datetime
    ) -> bool:
        """
    ...
    def list_feeds(self):
        ...
        for topic, title, feed_type, url, active in cursor.fetchall():
            ...
            feeds_by_topic[topic].append(
                {"title": title, "type": feed_type, "url": url, "active": bool(active)}
            )
