...
class RetentionCleanup:
            ...
            for transcript_file in transcript_dir.glob("*.txt"):
                try:
                    ...
                    if file_age < self.cutoff_timestamp:
                        ...
                        logger.info(f"  🗑️  Removing old transcript: {transcript_file.name} ({file_size:,} bytes)")
                        transcript_file.unlink()
                        removed_count += 1
                        
                except Exception as e:
                    logger.warning(f"  ⚠️  Could not process {transcript_file.name}: {e}")
        
        logger.info(f"✅ Removed {removed_count} transcript files ({total_size:,} bytes freed)")
    ...
    def cleanup_digest_files(self) -> Tuple[int, int]:
            ...
            for digest_file in digest_dir.glob(f"*{ext}"):
                try:
                    ...
                    if file_age < self.cutoff_timestamp:
                        ...
                        logger.info(f"  🗑️  Removing old digest file: {digest_file.name} ({file_size:,} bytes)")
                        digest_file.unlink()
                        removed_count += 1
        ...
        logger.info(f"✅ Removed {removed_count} digest files ({total_size:,} bytes freed)")
        return removed_count, total_size
    ...
    def cleanup_database_heavy_fields(self, db_path: str) -> Tuple[int, int]:
        ...
        try:
            ...
            if old_episodes:
                ...
                logger.info(f"  🗄️  Cleaned heavy fields from {updated_count} episodes ({bytes_freed:,} chars freed)")
            else:
    ...
    def run_full_cleanup(self) -> dict:
        ...
        episodes_cleaned, bytes_freed = self.cleanup_database_heavy_fields("podcast_monitor.db")
        results['rss_episodes_cleaned'] = episodes_cleaned
        results['rss_bytes_freed'] = bytes_freed
        
        # 4. Clean YouTube database heavy fields
        episodes_cleaned, bytes_freed = self.cleanup_database_heavy_fields("youtube_transcripts.db")
        ...
        vacuum_success = self.vacuum_databases(["podcast_monitor.db", "youtube_transcripts.db"])
        results['vacuum_success'] = vacuum_success
        
        # Summary
        total_files = results['transcript_files_removed'] + results['digest_files_removed']
        total_bytes = (results['transcript_bytes_freed'] + results['digest_bytes_freed'] + 
                      results['rss_bytes_freed'] + results['youtube_bytes_freed'])
        ...
        logger.info(f"  🧹 Database vacuum: {'✅ Success' if vacuum_success else '❌ Failed'}")
...
def main():
    ...
    total_files = results['transcript_files_removed'] + results['digest_files_removed']
    if total_files > 0 or results['rss_episodes_cleaned'] > 0 or results['youtube_episodes_cleaned'] > 0:
        print(f"🗑️  Retention cleanup completed: {total_files} files removed, {results['rss_episodes_cleaned'] + results['youtube_episodes_cleaned']} episodes cleaned")
        return 0
    else:
