...
class RobustTranscriber:
    ...
    def estimate_transcription_time(self, audio_file: str) -> Tuple[float, float, int]:
        ...
        try:
            ...
            if num_chunks > 1:
                chunking_overhead = (
                    num_chunks * 15
                )  # 15 seconds per chunk overhead (more realistic)
    ...
    def split_audio_file(self, input_file: str, chunk_duration: int = 300) -> List[str]:
        ...
        try:
            ...
            while start_time < duration:
                ...
                if (
                    chunk_file.exists() and chunk_file.stat().st_size > 1000
                ):  # Minimum 1KB
        ...
        while True:
            try:
                ...
                if elapsed - last_report >= 15:
                    ...
                    print(
                        f"🔄 Chunk {chunk_num}/{total_chunks} - Progress: {progress_pct:.1f}% "
    ...
    def transcribe_chunk(
        self, chunk_file: str, chunk_num: int, total_chunks: int
    ) -> Optional[str]:
        """
        ...
        try:
            ...
            print(
                f"📊 Chunk duration: {duration/60:.1f}min, estimated processing: {estimated_time:.0f}s"
            )
            ...
            print(
                f"✅ Chunk {chunk_num} complete: {processing_time:.1f}s actual "
    ...
    def transcribe_file(self, audio_file: str) -> Optional[str]:
        ...
        try:
            ...
            duration, estimated_time, recommended_chunks = (
                self.estimate_transcription_time(audio_file)
            )
            ...
            chunks = self.split_audio_file(
                audio_file, chunk_duration=600
            )  # 10-minute chunks
