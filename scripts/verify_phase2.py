...
class Phase2Verifier:
        ...
        for category_name, verify_func in verification_categories:
            ...
            try:
                ...
                if not category_result["passed"]:
                    all_passed = False
                    self.results["failed_checks"].extend(
                        category_result.get("failures", [])
                    )

                logger.info(
                    f"{'✅' if category_result['passed'] else '❌'} {category_name}: {category_result['status']}"
                )
    ...
    def verify_file_structure(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "status": (
                ...
                if len(failures) == 0
                else f"{len(failures)} files missing/empty"
            ),
            "failures": failures,
            "details": {
    ...
    def verify_code_migration(self) -> Dict[str, Any]:
        ...
        for file_path in files_to_check:
            ...
            has_gpt5 = (
                "gpt-5" in content
                or "gpt-5-mini" in content
                or "GPT5_MODELS" in content
                or "config.GPT5_MODELS['" in content
            )
            if not has_gpt5:
            ...
            if (
                "client.responses.create" not in content
                and "call_openai_with_backoff" not in content
            ):
                failures.append(f"No Responses API usage found in {file_path}")

            # Check for old GPT-4 references
            if "gpt-4" in content and "gpt-4" not in content.replace(
                "gpt-4", ""
            ...
            if "claude-" in content.lower() or (
                "claude" in content.lower()
                and "model" in content.lower()
                and "claude.ai" not in content.lower()
            ):
                failures.append(f"Claude model references still present in {file_path}")

            # Check for OpenAI helpers import
            if (
                "from utils.openai_helpers import" in content
                or "import utils.openai_helpers" in content
            ):
        ...
        return {
            "passed": len(failures) == 0,
            "status": (
                "Code properly migrated to GPT-5"
                if len(failures) == 0
                else f"{len(failures)} migration issues"
            ),
            "failures": failures,
            "details": {
    ...
    def verify_configuration(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "status": (
                ...
                if len(failures) == 0
                else f"{len(failures)} config issues"
            ),
            "failures": failures,
            "details": {"config_module": "config"},
        }
    ...
    def verify_database_schema(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "status": (
                ...
                if len(failures) == 0
                else f"{len(failures)} schema issues"
            ),
            "failures": failures,
            "details": {
    ...
    def verify_api_integration(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "status": (
                ...
                if len(failures) == 0
                else f"{len(failures)} API issues"
            ),
            "failures": failures,
            "details": {
                "tested_components": [
    ...
    def verify_test_coverage(self) -> Dict[str, Any]:
        ...
        if not test_file.exists():
            ...
            return {"passed": False, "status": "No test coverage", "failures": failures}
        ...
        return {
            ...
            "status": (
                ...
                if len(failures) == 0
                else f"{len(failures)} test issues"
            ),
            "failures": failures,
            "details": {
    ...
    def verify_documentation(self) -> Dict[str, Any]:
            ...
            if not doc_path.exists():
                ...
                if doc_file == "PHASED_REMEDIATION_TASKS.md":
                    if "Phase 2" not in content:
                        failures.append(
                            f"Documentation {doc_file} missing Phase 2 information"
                        )
        ...
        return {
            ...
            "status": (
                ...
                if len(failures) == 0
                else f"{len(failures)} doc issues"
            ),
            "failures": failures,
            "details": {"checked_docs": len(docs_to_check)},
        }
    ...
    def generate_summary(self):
        ...
        passed_categories = sum(
            1 for cat in self.results["categories"].values() if cat["passed"]
        )
        ...
        self.results["summary"] = {
            ...
            "overall_success_rate": (
                f"{(passed_categories / total_categories * 100):.1f}%"
    ...
    def print_results(self):
        ...
        print(f"\n📈 Summary:")
        print(
            f"   Categories: {summary['passed_categories']}/{summary['total_categories']} passed ({summary['overall_success_rate']})"
        )
