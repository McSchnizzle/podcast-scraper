...
class SchemaIntegrityVerifier:
    ...
    def verify_all(self, strict: bool = True) -> bool:
        ...
        try:
            with get_connection(self.db_path) as conn:
                ...
                return self.results['overall_status'] in ['passed', 'warning'] if not strict else self.results['overall_status'] == 'passed'
    ...
    def _check_foreign_key_enforcement(self, cursor: sqlite3.Cursor) -> None:
        ...
        if fk_status != 1:
            self._add_error("Foreign key enforcement is not enabled (PRAGMA foreign_keys != 1)")
        else:
    ...
    def _check_foreign_key_integrity(self, cursor: sqlite3.Cursor) -> None:
        ...
        if violations:
            self._add_error(f"Foreign key violations found: {len(violations)} violations")
    ...
    def _check_schema_version(self, cursor: sqlite3.Cursor) -> None:
        ...
        self.results['checks']['schema_version'] = {
            'status': 'passed' if current_version == EXPECTED_SCHEMA_VERSION else 'failed',
        ...
        if current_version != EXPECTED_SCHEMA_VERSION:
            if current_version == 0:
                self._add_warning(f"Schema version not set (user_version=0). Expected: {EXPECTED_SCHEMA_VERSION}")
            else:
                self._add_error(f"Schema version mismatch: {current_version}, expected: {EXPECTED_SCHEMA_VERSION}")
        else:
    ...
    def _check_required_indexes(self, cursor: sqlite3.Cursor) -> None:
        ...
        existing_indexes = {row[0]: {'table': row[1], 'sql': row[2]} for row in cursor.fetchall()}
        ...
        if missing_indexes:
            self._add_error(f"Missing required indexes: {[idx[0] for idx in missing_indexes]}")
        else:
            logger.info(f"✅ All required indexes exist ({len(existing_indexes)} indexes)")
    ...
    def _check_unique_constraints(self, cursor: sqlite3.Cursor) -> None:
        ...
        """)
        unique_constraints = {row[0]: {'table': row[1], 'sql': row[2]} for row in cursor.fetchall()}
        ...
        if missing_uniques:
            ...
            logger.info(f"✅ Unique constraints in place ({len(unique_constraints)} constraints)")
    
    def _check_orphaned_records(self, cursor: sqlite3.Cursor) -> None:
        """Check for orphaned records"""
        
        orphan_queries = [
            ("orphaned_episodes", "SELECT COUNT(*) FROM episodes e WHERE e.feed_id IS NOT NULL AND NOT EXISTS (SELECT 1 FROM feeds f WHERE f.id = e.feed_id)"),
            ("orphaned_episode_failures", "SELECT COUNT(*) FROM episode_failures ef WHERE NOT EXISTS (SELECT 1 FROM episodes e WHERE e.id = ef.episode_pk)"),
            ("orphaned_item_seen", "SELECT COUNT(*) FROM item_seen s WHERE NOT EXISTS (SELECT 1 FROM feeds f WHERE f.id = s.feed_id)"),
            ("orphaned_feed_metadata", "SELECT COUNT(*) FROM feed_metadata fm WHERE NOT EXISTS (SELECT 1 FROM feeds f WHERE f.id = fm.feed_id)"),
        ]
    ...
    def _check_data_integrity(self, cursor: sqlite3.Cursor) -> None:
        ...
        if null_episode_ids > 0:
            integrity_issues.append(f"Episodes with NULL/empty episode_id: {null_episode_ids}")
        ...
        if duplicate_episode_ids > 0:
            integrity_issues.append(f"Duplicate episode_id values: {duplicate_episode_ids}")
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: python verify_schema_integrity.py <database_path> [--strict] [--output report.json]")
        print("       python verify_schema_integrity.py --all [--strict] [--output-dir reports/]")
        sys.exit(1)
