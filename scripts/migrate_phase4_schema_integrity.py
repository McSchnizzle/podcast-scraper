...
class Phase4SchemaMigrator:
    ...
    def migrate(self, dry_run: bool = False) -> bool:
        ...
        try:
            ...
            if not dry_run and self.backup_path:
                logger.info(
                    f"Restore from backup: cp {self.backup_path} {self.db_path}"
                )
            return False
    ...
    def _create_backup(self) -> None:
        ...
        self.backup_path = (
            self.db_path.parent
            / f"{self.db_path.stem}.backup_pre_migration_{timestamp}"
        )
    ...
    def _check_data_quality(self, cursor: sqlite3.Cursor) -> None:
        ...
        cursor.execute(
            "SELECT name FROM sqlite_master WHERE type='table' AND name='episode_failures'"
        )
        if cursor.fetchone():
        ...
        with get_connection(self.db_path) as conn:
            ...
            if cursor.fetchone():
                ...
                if "episode_pk" in columns and "episode_id" not in columns:
                    ...
                    logger.info(
                        "Migration appears complete but schema version not updated"
                    )
    ...
    def _migrate_episode_failures(self, cursor: sqlite3.Cursor) -> None:
        ...
        old_count = cursor.execute("SELECT COUNT(*) FROM episode_failures").fetchone()[
            0
        ]
        new_count = cursor.execute(
            "SELECT COUNT(*) FROM episode_failures_new"
        ).fetchone()[0]
        dropped = old_count - new_count

        if dropped > 0:
            logger.warning(
                f"Dropped {dropped} orphaned episode_failures records during migration"
            )
    ...
    def _add_unique_constraints(self, cursor: sqlite3.Cursor) -> None:
        ...
        if cursor.fetchone():
            try:
                ...
                logger.warning(
                    f"  Failed to create unique constraint on episodes.episode_id: {e}"
                )
        ...
        if cursor.fetchone():
            ...
            if "item_id_hash" in columns:
                try:
                    ...
                    logger.info(
                        "  Added unique constraint: item_seen(feed_id, item_id_hash)"
                    )
                except sqlite3.Error as e:
                    logger.warning(
                        f"  Failed to create unique constraint on item_seen: {e}"
                    )
...
def main():
    ...
    if len(sys.argv) < 2:
        print(
            "Usage: python migrate_phase4_schema_integrity.py <database_path> [--dry-run]"
        )
    ...
    logger.info(f"\n{'='*60}")
    logger.info(
        f"Migration summary: {success_count}/{len(databases)} databases migrated successfully"
    )
