...
def migrate_table_timestamps(cursor, table_name: str, dry_run: bool = False) -> dict:
    ...
    try:
        ...
        for col_info in columns:
            ...
            if any(pattern in col_name for pattern in ['published', 'created', 'updated', 'timestamp', 'date']):
        ...
        for col_name in timestamp_columns:
            col_summary = migrate_column_timestamps(cursor, table_name, col_name, dry_run)
...
def migrate_column_timestamps(cursor, table_name: str, col_name: str, dry_run: bool = False) -> dict:
    """
    ...
    try:
        ...
        cursor.execute(f"SELECT rowid, {col_name} FROM {table_name} WHERE {col_name} IS NOT NULL AND {col_name} != ''")
        ...
        for rowid, timestamp_str in rows:
            try:
                ...
                normalized_timestamp, conversion_type = normalize_timestamp(timestamp_str)
                
                if normalized_timestamp and normalized_timestamp != timestamp_str:
                    # Track conversion types
                    col_summary["conversion_types"][conversion_type] = col_summary["conversion_types"].get(conversion_type, 0) + 1
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Migrate database timestamps to UTC format')
    ...
    for db_path in args.databases:
        ...
        print(f"✅ {db_path}: {result['total_rows_updated']} rows {'would be updated' if args.dry_run else 'updated'}")
            ...
            if table["rows_updated"] > 0:
                ...
                for col in table["columns_processed"]:
                    if col["rows_updated"] > 0:
                        conversions = ", ".join([f"{k}:{v}" for k, v in col["conversion_types"].items()])
