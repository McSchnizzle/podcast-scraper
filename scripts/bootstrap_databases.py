...
class DatabaseBootstrap:
    ...
    def bootstrap_rss_database(self) -> bool:
        ...
        try:
            ...
            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_episodes_digest_date ON episodes (digest_date)"
            )
            cursor.execute(
            ...
            logger.info(
                f"✅ RSS database initialized: {feeds_count} feeds, {episodes_count} episodes"
            )
            return True
    ...
    def bootstrap_youtube_database(self) -> bool:
        ...
        try:
            ...
            logger.info(
                f"✅ YouTube database initialized: {feeds_count} feeds, {episodes_count} episodes"
            )
            return True
    ...
    def verify_database_integrity(self, db_path: str) -> bool:
        ...
        try:
            ...
            missing_feeds_columns = [
                col for col in required_feeds_columns if col not in feeds_columns
            ]

            if missing_feeds_columns:
                logger.error(
                    f"❌ Missing required feeds columns: {missing_feeds_columns}"
                )
                conn.close()
                return False
            ...
            missing_episodes_columns = [
                col for col in required_episodes_columns if col not in episodes_columns
            ]

            if missing_episodes_columns:
                logger.error(
                    f"❌ Missing required episodes columns: {missing_episodes_columns}"
                )
                conn.close()
                return False
            ...
            logger.info(
                f"📊 Database info: {metadata.get('database_type', 'unknown')} schema v{metadata.get('schema_version', 'unknown')}"
            )
            logger.info(
                f"📊 Bootstrapped: {metadata.get('bootstrapped_at', 'unknown')}"
            )
            ...
            logger.info(
                f"📊 Table counts: {feeds_count} feeds, {episodes_count} episodes"
            )
    ...
    def run_full_bootstrap(self, skip_existing: bool = True) -> bool:
        ...
        if skip_existing and rss_exists and youtube_exists:
            ...
            if rss_valid and youtube_valid:
                ...
                logger.warning(
                    "⚠️ Existing databases have issues - continuing with bootstrap"
                )
...
def main():
    ...
    parser.add_argument(
        "--force", action="store_true", help="Force bootstrap even if databases exist"
    )
    parser.add_argument(
        "--verify-only", action="store_true", help="Only verify existing databases"
    )
    parser.add_argument(
        "--rss-only", action="store_true", help="Bootstrap RSS database only"
    )
    parser.add_argument(
        "--youtube-only", action="store_true", help="Bootstrap YouTube database only"
    )
    ...
    if args.rss_only:
        ...
        success = (
            bootstrap.bootstrap_youtube_database() and bootstrap.seed_youtube_feeds()
        )
        if success:
