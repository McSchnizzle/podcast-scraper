...
class SystemVerificationSuite:
    ...
    def verify_ingestion_transcription(self):
        ...
        try:
            ...
            if rss_episodes:
                ...
                logger.info(
                    f"✅ Found RSS episode flow: {sample_episode[4]} → {sample_episode[5]}"
                )
        ...
        try:
            ...
            if (
                "min_youtube_minutes" in processor_code
                and "duration_minutes < self.min_youtube_minutes" in processor_code
            ):
    ...
    def verify_topic_selection(self):
        ...
        try:
            ...
            if (
                "max_episodes_per_topic" in generator_code
                and "relevance_threshold" in generator_code
            ):
    ...
    def verify_openai_models(self):
        ...
        try:
            ...
            if "config.OPENAI_SETTINGS['digest_model']" in digest_code:
                self.evidence["model_configuration"][
                    "usage_location"
                ] = "openai_digest_integration.py:604 - Uses config.OPENAI_SETTINGS['digest_model']"
    ...
    def verify_rss_generation(self):
        ...
        try:
            ...
            if (
                "enclosure.set('length'" in rss_code
                and "enclosure.set('type', 'audio/mpeg')" in rss_code
            ):
    ...
    def verify_weekly_monday(self):
        ...
        try:
            ...
            if (
                "_generate_weekly_digest" in pipeline_code
                and "_generate_catchup_digest" in pipeline_code
            ):
    ...
    def verify_prose_validation(self):
        ...
        try:
            ...
            if (
                "bullet_patterns" in validator_code
                and "rewrite_to_prose" in validator_code
            ):
            ...
            if Path("bootstrap_databases.py").exists():
                ...
                self.evidence["database_bootstrap"] = {
                    ...
                    "required_columns_present": all(
                        col in found_columns for col in required_columns
                    ),
    ...
    def test_over_budget_selection(self):
        ...
        try:
            ...
            if (
                "max_reduce_tokens" in digest_code
                and "episode_summaries.pop()" in digest_code
            ):
                    ...
                    "sample_enclosure": (
                        {
                            ...
                            "length": (
                                enclosures[0].get("length") if enclosures else None
                            ),
        ...
        try:
            ...
            if workflow_file.exists():
                ...
                self.test_results["ci_validation"] = {
                    ...
                    "rss_generation": "rss_generator_multi_topic.py"
                    in workflow_content,
    ...
    def generate_evidence_bundle(self):
        ...
        bundle_file = (
            self.evidence_dir
            / f"evidence_bundle_{now_utc().strftime('%Y%m%d_%H%M%S')}.json"
        )
            ...
            if digest_dir.exists():
                recent_digests = len(
                    [
                        ...
                        if (now_utc() - datetime.fromtimestamp(f.stat().st_mtime)).days
                        < 7
                    ]
                )
