...
class TestPhase4FeedHelpers(unittest.TestCase):
    ...
    def test_item_identity_hash(self):
        ...
        hash1 = item_identity_hash(
            "guid-123", "http://example.com", "Title", "http://audio.com"
        )
        hash2 = item_identity_hash(
            "GUID-123", "http://different.com", "Different", "http://other.com"
        )
        ...
        hash_url1 = item_identity_hash(
            None, "http://example.com/path/", "Title", "http://audio.com"
        )
        hash_url2 = item_identity_hash(
            None, "HTTP://EXAMPLE.COM/path", "Different", "http://other.com"
        )
        hash_url3 = item_identity_hash(
            None, "http://example.com/path/?utm_source=test", None, None
        )
        self.assertEqual(
            hash_url1, hash_url2
        ...
        self.assertNotEqual(
            hash_url1, hash_url3
        )  # UTM parameters should make different hash
    ...
    def test_cleanup_old_item_seen(self):
        ...
        # Check results
        cursor.execute(
            "SELECT item_id_hash FROM item_seen WHERE feed_id = ?", (self.feed_id,)
        )
...
class TestPhase4HTTPCaching(unittest.TestCase):
    ...
    def test_cache_header_storage(self):
        ...
        # Store cache headers
        update_feed_cache_headers(
            cursor, feed_id, '"abc123"', "Wed, 01 Jan 2025 12:00:00 GMT"
        )
...
class TestPhase4FeedMonitor(unittest.TestCase):
    ...
    def test_database_initialization(self):
        ...
        cursor.execute(
            "SELECT name FROM sqlite_master WHERE type='table' AND name='feed_metadata'"
        )
        self.assertIsNotNone(cursor.fetchone())

        # Check item_seen table
        cursor.execute(
            "SELECT name FROM sqlite_master WHERE type='table' AND name='item_seen'"
        )
        self.assertIsNotNone(cursor.fetchone())

        # Check indexes
        cursor.execute(
            "SELECT name FROM sqlite_master WHERE type='index' AND name='ix_item_seen_feed_last_seen'"
        )
        self.assertIsNotNone(cursor.fetchone())
    ...
    def test_feed_metadata_creation(self):
        ...
        cursor.execute(
            "SELECT has_dates, typical_order FROM feed_metadata WHERE feed_id = ?",
            (feed_id,),
        )
    ...
    def test_enhanced_duplicate_detection(self, mock_parse, mock_get):
        ...
        for i in range(2):
            ...
            entry.get = lambda key, default=None, entry=entry: getattr(
                entry, key, default
            )
    ...
    def test_telemetry_integration(self, mock_telemetry):
        ...
        metric_calls = [
            call[0][0] for call in mock_telemetry_instance.record_metric.call_args_list
        ]
        expected_metrics = [
    ...
    def test_two_line_logging_format(self, mock_parse, mock_get):
        ...
        with patch("feed_monitor.logger") as mock_logger:
            ...
            for call in mock_logger.info.call_args_list:
                ...
                if (
                    re.search(r"Feed monitoring (started|completed)", call_str)
                    or re.search(r"📦.*Test Feed", call_str)  # Feed header line
                    or re.search(r"new=\d+.*updated=\d+.*\(\d+ms\)", call_str)
                ):  # Feed stats line
