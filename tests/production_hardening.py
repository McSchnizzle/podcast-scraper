...
class ProductionHardeningTests:
    ...
    def run_all_tests(self) -> Dict:
        ...
        for test in tests:
            try:
                ...
                logger.info(
                    f"✅ {test_name}: {'PASS' if result['status'] == 'pass' else 'FAIL'}"
                )
    ...
    def test_guid_immutability(self) -> Dict:
        ...
        return {
            ...
            "issue": (
                "Content-based stable GUIDs implemented"
                if guid_immutable
                else "GUIDs still use runtime timestamp"
            ),
            "recommendation": (
                "GUID stability fixed - same content produces same GUID"
                if guid_immutable
                else "Use content hash or episode metadata for stable GUIDs"
            ),
            "evidence": {
    ...
    def test_base_url_configuration(self) -> Dict:
        ...
        env_config_analysis = {
            ...
            "unique_domains": list(
                set([url.split("/")[2] for url in hardcoded_urls if "/" in url])
            ),
        }

        # Check if environment-driven configuration is working
        env_config_working = (
            env_base_url is not None
            and generator_default.config.PODCAST_BASE_URL == env_base_url
        )
        ...
        return {
            ...
            "issue": (
                ...
                if env_config_working
                else "Environment variables not properly configured"
            ),
            "recommendation": (
                "Production environment variables are now being used correctly"
                if env_config_working
                else "Set PODCAST_BASE_URL and AUDIO_BASE_URL environment variables"
            ),
            "evidence": {
                "env_config_active": env_config_working,
                "env_base_url": env_base_url,
                "config_base_url": generator_default.config.PODCAST_BASE_URL,
                "multiple_domains_in_feed": len(env_config_analysis["unique_domains"])
                > 1,
            },
        }
    ...
    def test_rss_spec_conformance(self) -> Dict:
        ...
        try:
            ...
            conformance_checks = {
                ...
                "has_atom_link": channel.find(".//{http://www.w3.org/2005/Atom}link")
                is not None,
                "guid_permalinks": [],
                "pubdate_consistency": [],
                "enclosure_validity": [],
            ...
            for item in items:
                ...
                if guid_elem is not None:
                    ...
                    conformance_checks["guid_permalinks"].append(
                        is_permalink == "false"
                    )
            ...
            passed_checks = (
                ...
                + sum(
                    [
                        enc["has_length"]
                        for enc in conformance_checks["enclosure_validity"]
                    ]
                )
            )
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                if result1 and result2:
                    ...
                    build_date1 = re.search(
                        r"<lastBuildDate>([^<]+)</lastBuildDate>", content1
                    )
                    build_date2 = re.search(
                        r"<lastBuildDate>([^<]+)</lastBuildDate>", content2
                    )
                    ...
                    return {
                        ...
                        "build_dates_differ": (
                            build_date1.group(1) if build_date1 else None
                        )
                        != (build_date2.group(1) if build_date2 else None),
                ...
                return {"status": "error", "critical": True, "error": str(e)}
    ...
    def test_security_boundaries(self) -> Dict:
        ...
        for filename in dangerous_filenames:
            try:
                ...
                sanitization_results.append(
                    {"original": filename, "error": str(e), "safe": False}
                )
                ...
                xml_sanitization_results.append(
                    {
                        ...
                        "safe": "<script>" not in safe_content
                        and "&" in safe_content.replace("&amp;", ""),
                        "preserves_content": len(safe_content) > 0,
                    }
                )
            except Exception as e:
                xml_sanitization_results.append(
                    {"original": content, "error": str(e), "safe": False}
                )

        # Security score
        file_safety_score = sum([r["safe"] for r in sanitization_results]) / len(
            sanitization_results
        )
        xml_safety_score = sum([r["safe"] for r in xml_sanitization_results]) / len(
            xml_sanitization_results
        )
        ...
        return {
            "status": (
                "pass"
                if file_safety_score >= 0.9 and xml_safety_score >= 0.9
                else "fail"
            ),
            "critical": True,
            ...
            "evidence": {
                "path_traversal_blocked": all(
                    "../" not in r.get("sanitized", "") for r in sanitization_results
                ),
                "xml_injection_blocked": all(
                    "<script>" not in r.get("sanitized", "")
                    for r in xml_sanitization_results
                ),
            },
        }
    ...
    def test_audio_metadata_integrity(self) -> Dict:
        ...
        for mp3_file in sample_files:
            try:
                ...
                audio_tests.append(
                    {"file": mp3_file.name, "error": str(e), "valid": False}
                )
        ...
        return {
            ...
            "evidence": {
                ...
                "has_metadata_extraction": any(
                    t.get("has_duration") for t in audio_tests
                ),
            },
        }
    ...
    def test_xml_injection_protection(self) -> Dict:
        ...
        for title in malicious_inputs:
            try:
                ...
                xml_safety_results.append(
                    {
                        ...
                        "sanitized": safe_title[:50]
                        + ("..." if len(safe_title) > 50 else ""),
                        "xml_valid": True,
                ...
                xml_safety_results.append(
                    {"original": title[:50], "error": str(e), "xml_valid": False}
                )

        safety_score = sum(
            [r.get("xml_valid", False) for r in xml_safety_results]
        ) / len(xml_safety_results)
        ...
        return {
            ...
            "evidence": {
                ...
                "length_limits_enforced": all(
                    r.get("length_controlled", True) for r in xml_safety_results
                ),
            },
        }
    ...
    def _check_atomic_operations(self) -> bool:
        ...
        for filename in key_files:
            try:
                ...
                if file_path.exists():
                    content = file_path.read_text()
                    if "temp" in content.lower() and (
                        "rename" in content.lower() or "move" in content.lower()
                    ):
                        return True
            except:
                continue
        ...
        for test_name, result in self.test_results.items():
            if result.get("status") == "fail" and result.get("critical"):
                ...
                recommendations.append(
                    {"test": test_name, "recommendation": result.get("recommendation")}
                )
        ...
        return {
            "summary": {
                ...
                "status": (
                    ...
                    else "PASS" if overall_score >= 0.8 else "NEEDS_WORK"
                ),
            },
...
def main():
    ...
    report_file = (
        Path(__file__).parent / "evidence" / f"production_hardening_{timestamp}.json"
    )
