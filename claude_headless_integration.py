    ...
    def _check_claude_availability(self) -> bool:
        ...
        try:
            result = subprocess.run(
                [self.claude_cmd, "--help"], capture_output=True, text=True, timeout=10
            )
    ...
    def get_transcripts_for_analysis(self) -> List[Dict]:
        ...
        try:
            ...
            for row in all_rows:
                ...
                logger.info(
                    f"Processing episode {episode_id}: '{title}' (status: {status}, transcript: {transcript_path})"
                )
                ...
                if transcript_path and Path(transcript_path).exists():
                    ...
                    logger.warning(
                        f"❌ Transcript file not found for episode {episode_id}: {transcript_path}"
                    )
...
"""
        ...
        for transcript in transcripts:
            ...
            try:
                ...
                logger.error(
                    f"Error loading transcript {transcript['transcript_path']}: {e}"
                )
    ...
    def run_claude_analysis(
        self, input_text: str, output_format: str = "text"
    ) -> Optional[str]:
...
"""
    ...
    def generate_claude_daily_digest(self) -> Tuple[bool, Optional[str], Optional[str]]:
        ...
        if not self.claude_available:
            logger.error(
                "Claude Code CLI not available - cannot generate Claude digest"
            )
...
def main():
    ...
    parser.add_argument(
        "--test", action="store_true", help="Test Claude Code availability"
    )
    parser.add_argument(
        "--analyze", action="store_true", help="Run cross-reference analysis"
    )
    ...
    if args.analyze:
        ...
        if transcripts:
            print(
                f"📊 Analyzing {len(transcripts)} transcripts for cross-references..."
            )
            cross_refs = integration.detect_cross_references(transcripts)
            print(
                f"✅ Found cross-reference data: {len(cross_refs) if isinstance(cross_refs, dict) else 'analysis complete'}"
            )
        return 0

    if args.digest:
        success, digest_path, cross_refs_path = (
            integration.generate_claude_daily_digest()
        )
        if success:
