...
if IS_APPLE_SILICON and PARAKEET_MLX_AVAILABLE and not IS_GITHUB_ACTIONS:
    ...
    if not IS_GITHUB_ACTIONS:  # Only raise error in local dev, not in CI
        raise ImportError(
            "No ASR backend available. Install faster-whisper, whisper, or parakeet-mlx"
        )
...
class ContentProcessor:
    ...
    def process_episode(self, episode_id):
        ...
        ep_id, episode_guid, title, audio_url, feed_type, topic_category, feed_title = (
            episode
        )
        ...
        try:
            ...
            if transcript:
                ...
                if self.openai_scorer.api_available:
                    ...
                    try:
                        topic_scores = self.openai_scorer.score_transcript(
                        ...
                        if topic_scores and not topic_scores.get("error"):
                            ...
                            if topic_items:
                                top_topics = sorted(
                                    topic_items, key=lambda x: x[1], reverse=True
                                )[:2]
                ...
                conn.commit()
                print(
                    f"✅ Processed successfully - Priority: {analysis['priority_score']:.2f}"
                )
                ...
                if feed_type == "youtube" and transcript is None:
                    cursor.execute(
                        "UPDATE episodes SET status = ? WHERE id = ?", ("failed", ep_id)
                    )
                    conn.commit()
    ...
    def _process_youtube_episode(self, video_url, episode_id):
        ...
        try:
            ...
            if is_github_actions:
                print(
                    "🚨 Running in GitHub Actions - YouTube API may be blocked by cloud IPs"
                )
            ...
            if is_github_actions:
                ...
                if thread.is_alive():
                    raise TimeoutError(
                        "YouTube API request timed out in GitHub Actions"
                    )
            ...
            if is_github_actions:
                if (
                    ...
                    self._log_episode_failure(
                        video_url, f"GITHUB_ACTIONS_SKIP: {skip_reason}"
                    )
                    return None
    ...
    def _process_rss_episode(self, audio_url, episode_id):
        ...
        try:
            ...
            if not audio_file:
                self._log_episode_failure(
                    episode_id, "Audio download failed", "download"
                )
                return None

            # Update status to 'downloaded' after successful audio download
            print(f"✅ Audio downloaded to: {audio_file}")
            self._update_episode_status(episode_id, "downloaded")
    ...
    def _update_episode_status(self, episode_id, status, error_reason=None):
        ...
        try:
            ...
            if error_reason:
                ...
                print(
                    f"📊 Episode {episode_id} status: {status} (reason: {error_reason})"
                )
            else:
                cursor.execute(
                    """
    ...
    def _parakeet_mlx_transcribe(self, audio_file):
        ...
        try:
            ...
            transcript_text = self._add_basic_speaker_detection(
                transcript_text, audio_file
            )
    ...
    def _whisper_transcribe(self, audio_file):
        ...
        try:
            ...
            print(
                f"✅ Whisper transcription complete: {len(transcript_text)} characters"
            )
    ...
    def _faster_whisper_transcribe(self, audio_file):
        ...
        try:
            ...
            print(
                f"   • Estimated processing time: {estimated_total_time/60:.1f} minutes"
            )
            print(f"   • Target RTF: ~{estimated_rtf:.3f}x (4x faster than Whisper)")

            # Step 2: Split into chunks if needed with enhanced logging
            chunks = self._split_audio_for_faster_whisper(
                audio_file, max_chunk_duration
            )
            if not chunks:
            ...
            for i, chunk_file in enumerate(chunks, 1):
                ...
                chunk_transcript = self._transcribe_faster_whisper_chunk(
                    chunk_file, i, len(chunks)
                )
                ...
                if chunk_transcript:
                    ...
                    actual_rtf = (
                        chunk_processing_time / chunk_duration
                        if chunk_duration > 0
                        else 0
                    )
                    speedup = (
                        chunk_est_time / chunk_processing_time
                    ...
                    print(
                        f"   📊 Performance: {chunk_processing_time:.1f}s actual (RTF: {actual_rtf:.3f}x)"
                    )
                    ...
                    if i < len(chunks):
                        ...
                        print(
                            f"   📈 Overall Progress: {progress_pct:.1f}% ({i}/{len(chunks)} chunks)"
                        )
                        print(
                            f"   ⏰ Time: {elapsed_total/60:.1f}min elapsed, ~{est_remaining/60:.1f}min remaining"
                        )
                else:
            ...
            print(
                f"   • Output: {len(final_transcript):,} characters from {len(chunks)} chunks"
            )
            print(
                f"   • Average per chunk: {len(final_transcript)//len(chunks):,} chars"
            )

            # Add basic speaker detection
            final_transcript = self._add_basic_speaker_detection(
                final_transcript, audio_file
            )
    ...
    def _split_audio_for_faster_whisper(self, input_file, chunk_duration=600):
        ...
        try:
            ...
            if duration <= chunk_duration:
                print(
                    f"📄 File duration ({duration/60:.1f}min) within chunk limit - no splitting needed"
                )
                return [str(input_file)]

            print(
                f"✂️ Splitting {duration/60:.1f}min audio into {chunk_duration//60}min chunks..."
            )
            ...
            while start_time < duration:
                ...
                print(
                    f"📄 Creating chunk {chunk_num}: {start_time//60:.0f}m-{(start_time+chunk_duration)//60:.0f}m"
                )
                ...
                if (
                    ...
                    and chunk_file.stat().st_size > 1000
                ):
                    chunk_files.append(str(chunk_file))
                    print(
                        f"✅ Chunk {chunk_num} created: {chunk_file.stat().st_size//1024//1024}MB"
                    )
                else:
    ...
    def _transcribe_faster_whisper_chunk(self, chunk_file, chunk_num, total_chunks):
        ...
        try:
            ...
            if hasattr(info, "duration_after_vad"):
                ...
                vad_removed_pct = (
                    (vad_removed / info.duration) * 100 if info.duration > 0 else 0
                )
                print(
                    f"   🔇 VAD removed: {vad_removed:.1f}s ({vad_removed_pct:.1f}% silence)"
                )
    ...
    def process_retry_queue(self, max_retries_per_run=5):
        ...
        try:
            ...
            if results["processed"] > 0:
                print(
                    f"📊 Retry Results: {results['succeeded']} succeeded, {results['failed']} failed out of {results['processed']} processed"
                )
                ...
                for result in results["retry_results"]:
                    ...
                    print(
                        f"  {status} {result['episode_id']}: {result['title']} (attempt #{result['retry_attempt']})"
                    )
    ...
    def _add_basic_speaker_detection(self, transcript_text, audio_file):
        ...
        try:
            ...
            duration_estimate = file_size / (
                16000 * 2
            )  # rough estimate for 16kHz 16-bit
            ...
            short_responses = sum(
                1 for sentence in sentences if len(sentence.strip()) < 50
            )

            # Determine if likely multi-speaker based on heuristics
            is_likely_conversation = (
                duration_estimate > 300  # > 5 minutes
                and indicator_count > 5  # Conversational language
                and short_responses > 3  # Short back-and-forth responses
                and len(transcript_text) > 1000  # Substantial content
            )
    ...
    def _analyze_content(self, transcript, topic_category):
        ...
        high_matches = sum(
            1 for keyword in high_priority_keywords if keyword in transcript_lower
        )
        medium_matches = sum(
            1 for keyword in medium_priority_keywords if keyword in transcript_lower
        )
    ...
    def process_all_pending(self):
        ...
        cursor.execute(
            "SELECT id FROM episodes WHERE status IN ('pending', 'pre-download')"
        )
    ...
    def get_processing_stats(self):
        ...
        return {"by_source": stats, "min_youtube_minutes": self.min_youtube_minutes}
...
def main():
    ...
    if PARAKEET_MLX_AVAILABLE:
        print(
            "✅ Parakeet MLX available - using Apple Silicon optimized ASR for RSS transcription"
        )
    ...
    if results:
        ...
        if high_priority:
            ...
            for ep in high_priority[:5]:  # Show top 5
                print(f"  🎯 {ep['title']}")
                print(
                    f"     Priority: {ep['priority_score']:.2f} | Type: {ep['content_type']}"
                )
