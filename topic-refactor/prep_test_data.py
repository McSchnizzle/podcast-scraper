...
def main():
    p = argparse.ArgumentParser(description="Prep podcast DBs for multi-topic digest test (2 YouTube, 1 RSS).")
    ...
    p.add_argument("--youtube-ids", required=True, help="Comma-separated two YouTube episode IDs to keep, e.g. Y1,Y2")
    p.add_argument("--rss-id", required=True, help="Single RSS episode ID to keep, e.g. R1")
    p.add_argument("--transcripts-dir", default="transcripts", help="Directory for transcript files (default: transcripts)")
    p.add_argument("--dummy-transcripts", action="store_true", help="Create dummy transcript files for the YouTube episodes")
    p.add_argument("--mode", choices=["archive", "delete"], default="archive", help="Archive (status='archived') or delete other episodes (default: archive)")
    p.add_argument("--no-backup", action="store_true", help="Skip backup of DBs before modifying (not recommended)")
    args = p.parse_args()

    yids = [x.strip() for x in args.youtube_ids.split(",") if x.strip()]
    if len(yids) != 2:
        raise SystemExit("Please provide exactly TWO YouTube IDs via --youtube-ids (e.g., --youtube-ids Y1,Y2)")
    ...
    for i, yid in enumerate(yids, start=1):
        ...
        if args.dummy_transcripts:
            write_dummy(tpath, content=f"Dummy transcript for {yid}. This is a short snippet for testing the digest pipeline.")

    # Prune RSS DB to one ID
    if args.mode == "archive":
        exec_sql(args.rss_db, f"""
    ...
    y_rows = query(args.youtube_db, "SELECT id, status, transcript_path, digest_topic, digest_date FROM episodes ORDER BY id;")
    r_rows = query(args.rss_db, "SELECT id, status, transcript_path, digest_topic, digest_date FROM episodes ORDER BY id;")
