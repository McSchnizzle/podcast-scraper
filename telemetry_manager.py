...
class TelemetryManager:
    ...
    def record_topic_processing(self, 
                              topic: str,
                              total_candidates: int,
                              above_threshold_count: int,
                              selected_count: int,
        ...
        if map_phase_tokens > 0:
            ...
            cost_estimate = (map_phase_tokens * 0.00015 / 1000) + (map_phase_tokens * 0.0006 / 1000)
        else:
            cost_estimate = 0
            
        if reduce_phase_tokens > 0:
            # Reduce phase uses primary model  
            cost_estimate += (reduce_phase_tokens * 0.01 / 1000) + (reduce_phase_tokens * 0.03 / 1000)
        ...
        logger.info(f"    Candidates: {total_candidates} → Threshold ≥{threshold_used}: {above_threshold_count} → Selected: {selected_count}")
        logger.info(f"    Tokens: Map={map_phase_tokens} + Reduce={reduce_phase_tokens} = {total_tokens}")
        logger.info(f"    Retries: {retry_count}, Time: {processing_time:.1f}s")
        logger.info(f"    Episodes included: {len(episode_ids_included)}, dropped: {len(episode_ids_dropped)}")
        if error_message:
            logger.info(f"    Error: {error_message}")
    
    def record_processing_stats(self, 
                              transcribed: int = 0,
                              scored: int = 0, 
                              digested: int = 0,
                              deployed: int = 0,
    ...
    def _log_run_summary(self):
        ...
        logger.info(f"Episodes: {run.episodes_transcribed} transcribed, {run.episodes_scored} scored, {run.episodes_digested} digested")
        ...
        for topic_metrics in run.topics_processed:
            ...
            logger.info(f"  {status} {topic_metrics.topic}: {topic_metrics.selected_count}/{topic_metrics.total_candidates} episodes, {topic_metrics.total_tokens} tokens")
    ...
    def _cleanup_old_telemetry(self):
        ...
        for telemetry_file in self.telemetry_dir.glob("run_*.json"):
            try:
                ...
                date_str = telemetry_file.stem.split('_')[1] + '_' + telemetry_file.stem.split('_')[2]
        ...
        if removed_count > 0 or map_removed_count > 0:
            logger.info(f"🧹 Cleaned up {removed_count} old telemetry files and {map_removed_count} old map summaries")
    ...
    def generate_summary_report(self, days: int = 7) -> str:
        ...
        topic_success_rate = (successful_topics / total_topics * 100) if total_topics > 0 else 0
...
"""
        ...
        for i, run in enumerate(recent_runs[:5]):  # Show last 5 runs
            run_date = datetime.fromisoformat(run['timestamp']).strftime('%Y-%m-%d %H:%M')
            topics = len(run.get('topics_processed', []))
            success = len([t for t in run.get('topics_processed', []) if t.get('success', False)])
