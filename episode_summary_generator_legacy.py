...
class EpisodeSummaryGenerator:
    ...
    def _cache_summary(
        self, content_hash: str, topic: str, timestamp: str, summary: str, content: str
    ):
    ...
    def _clean_content_for_summary(self, content: str, max_words: int = 1500) -> str:
        ...
        if len(words) > max_words:
            ...
            logger.info(
                f"📝 Truncated content to {max_words} words for summary generation"
            )

        return content

    def generate_summary(
        self, content: str, topic: str, timestamp: str, fallback_desc: str = None
    ) -> str:
        ...
        if not content or len(content.strip()) < 50:
            ...
            return (
                fallback_desc
                or f"Daily digest episode covering {topic.replace('_', ' ')}"
            )
        ...
        if not self.api_available:
            logger.info(
                "🔄 OpenAI API not available, using enhanced fallback description"
            )
        ...
        try:
            ...
            logger.info(
                f"🤖 Generating AI summary for {topic} episode ({len(clean_content)} chars)"
            )
            ...
            if len(summary) < 20 or len(summary) > 300:
                logger.warning(
                    f"⚠️ Generated summary length unusual ({len(summary)} chars), using fallback"
                )
    ...
    def _generate_fallback_summary(
        self, content: str, topic: str, fallback_desc: str = None
    ) -> str:
    ...
    def generate_episode_summary(
        self, episode_data: Dict, topic: str, max_tokens: int = None
    ) -> Optional[str]:
...
Create a focused summary for {topic} digest."""
        ...
        try:
            response = self.client.chat.completions.create(
                model=self.settings[
                    "scoring_model"
                ],  # Use cost-effective model for efficiency
                messages=[
            ...
            logger.info(
                f"Generated summary for {episode_data['episode_id']} ({topic}): {actual_tokens} tokens"
            )
            ...
            logger.error(
                f"Failed to generate summary for {episode_data['episode_id']} ({topic}): {e}"
            )
            return None

    def select_top_episodes_for_topic(
        self,
        ...
        logger.info(
            f"Topic '{topic}': {len(relevant_episodes)} candidates, "
    ...
    def generate_topic_summaries(
        self, transcripts: List[Dict], topic: str
    ) -> List[Dict]:
        ...
        for episode in selected_episodes:
            logger.info(
                f"Generating summary for {episode['episode_id']} (score: {episode['topic_score']:.3f})"
            )
        ...
        logger.info(
            f"Generated {len(summaries)} summaries for '{topic}': {total_tokens} total tokens"
        )
